...
//Find the reason of IntArrayRef not working?
at::Tensor myConv2d(
		at::Tensor& input,
		at::Tensor& weight,
		at::Tensor& bias,
		...
		)
{
   ...
   CHECK_INPUT(weight); 
   CHECK_INPUT(bias); 
   at::ArrayRef<int64_t> stride{cstride};
   at::ArrayRef<int64_t> padding{cpadding};   
   at::ArrayRef<int64_t> dilation{cdilation};
					      ...
					      int64_t groups)
{
    ...
    at::ArrayRef<int64_t> padding{cpadding};   
    at::ArrayRef<int64_t> dilation{cdilation};
    at::ArrayRef<int64_t> input_size{cinput_size};

    return(cudnn_convolution_backward_input(input_size,grad_output,weight,padding,stride,dilation,groups,benchmark,deterministic));
					      ...
					      int64_t groups)
{
    ...
    at::ArrayRef<int64_t> padding{cpadding};   
    at::ArrayRef<int64_t> dilation{cdilation};
    at::ArrayRef<int64_t> weight_size{cweight_size};
    
    return(cudnn_convolution_backward_weight(weight_size,grad_output,input,padding,stride,dilation,groups,benchmark,deterministic));
}
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> mycudnn_batch_norm(
    ...
    bool training, double exponential_average_factor, double epsilon)
{
	...
	if (training==true){
		...
		//std::cout<<std::get<1>(out);
		//std::cout<<std::get<2>(out);	
		auto val=std::get<0>(out);
		
		return std::make_tuple(val,save_mean,save_var);
	}
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> mycudnn_batch_norm_backward(
    ...
    double epsilon)
{
	return torch::native::cudnn_batch_norm_backward(input_t,grad_output_t,weight_t,running_mean,running_var,save_mean_t,save_var_t,epsilon);
}
