...
class MyBatchNorm(Module):
    ...
    def forward(self, input):
        ...
        if self.training or not self.track_running_stats:
            return mybatchnorm2d()( input, self.weight,self.bias, self.running_mean, self.running_var, torch.tensor(1),torch.tensor(exponential_average_factor), torch.tensor(self.eps))
        else:
            return mybatchnorm2d()( input, self.weight,self.bias, self.running_mean, self.running_var, torch.tensor(0),torch.tensor(exponential_average_factor), torch.tensor(self.eps))
...
class MyBatchNorm2d(MyBatchNorm):
    def _check_input_dim(self, input):
        if input.dim() != 4:
...
if __name__ == "__main__":            
    ...
    my_bn = MyBatchNorm2d(3, affine=True)#.to_device(device)
    ...
    class myModel(nn.Module):
        def __init__(self):
            ...
            self.batchnorm=MyBatchNorm2d(input_channel) 
